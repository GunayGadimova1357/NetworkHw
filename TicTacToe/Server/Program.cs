...
listener.Start();
Console.WriteLine($"[SERVER] Waiting for a client for {port}...");
...
Console.Clear();
Console.WriteLine("Choose your mark: 1 — X (crosses), 2 — O (noughts)");
char my = 'X';
while (true)
{
...
await writer.WriteLineAsync($"ASSIGN {my}");
char opp = my == 'X' ? 'O' : 'X';
char turn = 'X'; 
...
try
{
    while (running)
    {
        Draw(board, cx, cy, my, turn, isMyTurn: turn == my && !gameOver, status: "[SERVER]", gameOver, endMessage);
